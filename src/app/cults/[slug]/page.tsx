...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: `${entry.name} | Cults`,
    description: entry.short_bio ?? undefined,
  }
}

export default async function CultProfile({ params }: Props) {
  const { slug } = await params
  const entry = await getEntry(slug)
  if (!entry) notFound()
    ...
    <div>
      ...
      <div style={{ background: 'var(--bg-deep)', borderBottom: '1px solid var(--border)', padding: '0.75rem 0' }}>
        <div className="container" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Link href="/cults" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            Cults
          </Link>
      ...
      <div className="page-header">
        <div className="container">
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
            ...
            {entry.dissolved_year && (
              <span style={{
                fontFamily: 'var(--font-mono)', fontSize: '0.6rem', letterSpacing: '0.12em',
                textTransform: 'uppercase', color: 'var(--text-muted)',
                border: '1px solid var(--border)', padding: '0.2rem 0.5rem', borderRadius: '2px',
              }}>Dissolved {entry.dissolved_year}</span>
            )}
          </div>
            ...
            {entry.full_profile && (
              <div style={{
                ...
                <h2 style={{
                  ...
                  color: '#e8a020', marginBottom: '1.5rem',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {sources.map((source) => (
                    <div key={source.id} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#e8a020', border: '1px solid #5c3a00', padding: '0.15rem 0.4rem', borderRadius: '2px', flexShrink: 0, marginTop: '2px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '4px', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {[
                  { label: 'Founded', value: entry.founded_year },
                  { label: 'Dissolved', value: entry.dissolved_year },
                  { label: 'Founder', value: entry.founder },
                  { label: 'Country', value: entry.country },
                  { label: 'Headquarters', value: entry.headquarters },
                  { label: 'Peak Members', value: entry.membership_peak?.toLocaleString() },
                  { label: 'Ideology', value: entry.ideology },
                ].filter(s => s.value != null).map(({ label, value }) => (
                  <div key={label} className="stat-block" style={{ borderLeftColor: '#e8a020' }}>
            ...
            <Link href="/cults" className="btn btn-ghost" style={{ justifyContent: 'center' }}>
              ← Back to Database
            </Link>
          </div>
        </div>
